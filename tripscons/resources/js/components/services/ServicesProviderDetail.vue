          ...
          <div class="about-info mt-2 container p-4">
            ...
            <div class="interests d-flex mt-2">
              <span v-for="(i, index) in generalData.activity" :key="index">
                <img width="25px" :src="'/assets/img/icons/' + i.image" alt="Tripscon" />
                {{ i.name }}
              </span>
            </div>
          </div>
        ...
        <div class="col-12 col-sm-8 col-md-8">
          <div class="profile-right p-5">
            ...
            <div class="row country-grid mt-4">
              <div v-if="(guides).length > 0" v-for="(guide, index) in guides" class="col-sm-12 col-md-4 my-2">
                <router-link  :to="{path:'/guides/edit/'+guide.id}">
                  <div class="images-wrapper">
                
                    <div v-if="guide.single_image" class="location-image">
                      <img :src="`/assets/uploads/guides/${guide.single_image.name}`" alt="" />
                    </div>
                    <div v-else class="location-image">
                      <img src="/assets/uploads/not-available.png" alt="" />
                    </div>
                    

                    <div class="location-info mt-2">
                      <span class="hotel-name"> {{ guide.title }} </span>
...
export default {
  ...
  data() {
    return {
      ...
      activityCount: "",
      guides: "",
      Id: this.$route.params.serviceProviderId,
    };
  },
  created() {
    this.idGet();
  ...
  methods: {
    idGet() {
      axios.get("/api/getServiceProviderDetail/" + this.Id).then((response) => {
        //alert(1);
        this.generalData = response.data.data;
      //  console.log(this.generalData);
        this.guides = this.generalData.guides
