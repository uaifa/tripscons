        ...
        <div class="profile-right pb-5 px-5">
          ...
          <div class="stay-in-sec">
            <div class="d-flex justify-content-between my-2">
              <h3 class="title-section f-30">Meals</h3>
              <router-link :to="{ path: '/host/meals/add/step1' }">
            ...
            <div class="row">
              ...
              >
                <router-link
                  :to="{
                    path: '/host/meals/detail/' + meal.id,
                  }"
                >
                  <div class="hotel-box">
                    <div class="img-holder">
                      <img
                        :src="
                          $imagePath +
                          'meals/' +
                          meal.single_image.name
                        "
                        alt="hotel"
                        v-if="meal.single_image != null"
                      />
                    ...
                    <div class="content-holder">
                      ...
                      <a
                        href="#"
                        class="btn btn-price"
                        v-if="meal.price != null"
                        >{{ meal.price }}Rs / 
                        <span v-if="meal.unit=='KG'"> Per {{ meal.unit }}</span>
                        <span v-else> {{ meal.unit }}</span>
                        </a>
                    </div>
                  </div>
                </router-link>
          ...
          <pagination
            :data="meals"
            @pagination-change-page="getHostMeal"
          ></pagination>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getHostMeal(page) {
      ...
      axios.post("/api/getHostMeals", bodyFormData).then((response) => {
        this.userObj = response.data.data.host;
        this.meals = response.data.data.meal;
      });
    },
  },
};
