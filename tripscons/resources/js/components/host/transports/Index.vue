        ...
        <div class="profile-right pb-5 px-5">
          ...
          <div class="stay-in-sec">
            <div class="d-flex justify-content-between my-2">
              <h3 class="title-section f-30">Transports</h3>
              <router-link :to="{ path: '/host/transports/add/step1' }">
            ...
            <div class="row">
              <div
                ...
                v-for="(transport, index) in transports.data"
                :key="index"
              >
                <router-link
                  :to="{
                    path: '/host/transports/detail/' + transport.id,
                  }"
                >
                  <div class="hotel-box">
                    <div class="img-holder">
                      <img
                        :src="
                          $imagePath +
                          'transports/' +
                          transport.single_image.name
                        "
                        alt="hotel"
                        v-if="transport.single_image != null"
                      />
          ...
          <pagination
            :data="transports"
            @pagination-change-page="getHostTransport"
          ></pagination>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getHostTransport(page) {
      ...
      axios.post("/api/getHostTransports", bodyFormData).then((response) => {
        this.userObj = response.data.data.host;
        this.transports = response.data.data.transport;
      });
    },
  },
};
