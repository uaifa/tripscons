...
class MovieMaker implements iSearchFilter
{
    ...
    public function filter(Request $request)
    {
        ...
        $movieMakers = User::where('role_id', 9)->get();
        // Gender Filter
        if ($movieMakers && ($gender == 'male' || $gender == 'female')) {
            foreach ($movieMakers as $movieMaker) {
                if ($movieMaker->gender == $gender) {
                    array_push($filteredUsers, $movieMaker);
                }
            }
        } elseif ($movieMakers && $gender == 'no') {
            $filteredUsers = $movieMakers;
        }
        // Activities Filter
