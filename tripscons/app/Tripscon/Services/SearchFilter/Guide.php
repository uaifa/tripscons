...
class Guide implements iSearchFilter
{
    ...
    public function filter(Request $request)
    {
        ...
        $guides = User::where('role_id', 4)->get();
        // Gender Filter
        if ($guides && ($gender == 'male' || $gender == 'female')) {
            foreach ($guides as $guide) {
                if ($guide->gender == $gender) {
                    array_push($filteredUsers, $guide);
                }
            }
        } elseif ($guides && $gender == 'no') {
            $filteredUsers = $guides;
        }
        // Activities Filter
