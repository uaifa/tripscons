...
class MovieMaker implements iOrder
{

    /**
     * @Description Send Notification when user send proposal for Book Movie Maker
    ...
    public function sendNotificationBeforeBook(Request $request)
    {
        ...
        if ($request->dayOrHourly == 'day') {
            ...
            $orderItemsBody = [
                ['key' => 'date_form', 'value' => (string)date("Y-m-d", strtotime($request->date_from))],
                ['key' => 'date_to', 'value' => (string)date("Y-m-d", strtotime($request->date_to))],
