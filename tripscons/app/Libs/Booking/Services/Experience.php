...
class Experience implements Bookable {

    private $id;
    private $service;
    private $model = ModelsExperience::class;
    private $data;
    private $cart;
    private $subtotal;
    private $discounttotal;
    private $grandtotal;
    private $type = "Activity Booking";
    ...
    public function __construct($id, $data = [], $currency = 'PKR')
    {
        ...
        request()->validate([
            ...
            'data.adults' => 'integer',
            'data.children' => 'integer',
            'data.price' => 'nullable',
            'data.booking_type' => 'required|in:Solo,Private Group'
        ]);
    ...
    {
        $this->startDate = Carbon::create(request()->data['start_date'])->format("Y-m-d H:i:s");
        $this->endDate = Carbon::create($this->startDate)->add($this->service->duration)->format('Y-m-d H:i:s');
    }
...
// Notification Push to Admin Accomodation Booking request
    ...
    {
        $this->data['start_date'] = $this->startDate;
        $this->data['end_date'] = $this->endDate;
        $price = ((is_numeric($this->data['price']) ? $this->data['price'] : null) ?? $this->service->price);
        $this->cart['persons'] = $this->data['booking_type'] == 'Solo' ? 1 : $this->data['adults'] + $this->data['children'];
        $this->cart['location'] = $this->service->location;
        $this->cart['rent'] = $this->convertedAmount($price);
        $this->cart['items'] = [];
        ...
        $this->cart['items'][] = [
            "label" => "Experience",
            "desc" => $this->service->title . " Activity " . $this->currency->currency . " " . $this->convertedAmount($price) . " x " . $this->cart['persons'] . " persons",
            "price" => $this->convertedAmount($price * $this->cart['persons'])
        ];

        if(intval($this->data['children']) != 0 && intval($this->service->child_discount) != 0){
            $this->cart['discounts'][] =[
                "label" => "Child Discount",
                "desc" => "Child Discount (1 child only)",
                "price" => round((($this->cart['rent'] / 100) * $this->service->child_discount), 2)
            ];

            $this->cart['discounttotal'] += (($this->cart['rent'] / 100) * $this->service->child_discount);
        }
        ...
        if($this->service->is_offer_promotion_discount && $this->service->promotion_discount){
            $this->cart['discounts'][] =[
                ...
                "price" => round((($this->cart['rent'] / 100) * $this->service->promotion_discount) * $this->cart['persons'], 2)
            ];
            $this->cart['discounttotal'] += round((($this->cart['rent'] / 100) * $this->service->promotion_discount) * $this->cart['persons'], 2);
        }
