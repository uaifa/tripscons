...
class TripInquiry implements Bookable {

    private $id;
    private $service;
    private $model = ModelsInquiry::class;
    private $data;
    private $cart;
    private $subtotal;
    private $discounttotal;
    private $grandtotal;
    private $type = "Inquiry Booking";
    ...
    public function book()
    {
        if($this->data['instant'] ?? false){
            ...
            $dateFrom = $this->service->date_from;
            $dateTo = $this->service->date_to;
        }
        ...
        $reservation = Reservation::forceCreate([
            ...
            'bookable_id' => $this->id,
            'room_id' => null,
            'provider_user_id' => $this->data['provider_user_id'],
...
// Notification Push to Vendor Accomodation Booking request
        ...
        $vendor = \App\Models\User::where('id',$this->data['provider_user_id'])->first();
...
// Notification Push to Admin Accomodation Booking request
        ...
        $this->service->status=$this->model::CLOSED_INQUIRY;
        $this->service->save();
        return $reservation;
    }
    ...
    public function setCart()
    {
        ...
        $price = ((is_numeric($this->data['purposedBudget']) ? $this->data['purposedBudget'] : null) ?? $this->service->budget);
        $this->cart['price'] = $this->convertedAmount($price);
        $this->cart['items'] = [];
        // $this->cart['quantity'] = $this->service->persons;
        ...
        $this->cart['items'][] = [
            ...
            // "desc" => $this->service->title . " " . $this->convertedAmount($price) . " x " . $this->cart['quantity'],
             "desc" => $this->data['title'] . " " . $this->convertedAmount($price) . " x " . $this->cart['quantity'],
        ...
        // $tax = round($this->cart['subtotal'], 2) /100 * $taxPercentage;
        $tax = 0;
        ...
        $this->cart['grandtotal'] = round($this->cart['subtotal'] + $tax, 2);
