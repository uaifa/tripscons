...
class RoomAccommodation implements Bookable {

    private $id;
    private $service;
    private $model = Accommodation::class;
    private $data;
    private $cart;
    private $subtotal;
    private $discounttotal;
    private $grandtotal;
    private $type = "Hotel Room Booking";
    ...
    public function getService($id)
    {
        ...
        if($service->type_id == 0) throw new Exception("PROVIDED ID $id IS NOT COMPATIBLE WITH PROVIDED TYPE " . self::class);
...
//
//        $title      = "Room Accommodation Booking";
//        $message    = "Room Accommodation Booking request successfully";
//        $action     = "api/Room-Accomodation";
//
...
// Notification Push to Admin Accomodation Booking request
    ...
    public function setCart()
    {
        ...
        foreach($this->data['rooms'] as $room){
            $roominfo = Room::find($room['id']);
            $this->cart['items'][] = [
                "label" => $roominfo->title . " " .  $roominfo->room_type,
                "desc" => $room['roomQuantity'] . " x " . $roominfo->room_type . " (" . $this->cart['nights'] . " nights)",
                "price" => round($room['roomQuantity'] * $room['price'] * intval($this->cart['nights']))
            ];

            if($room['extraGuests'] > 0){
                $this->cart['items'][] = [
                    "label" => "Extra guest charges",
                    "desc" => $room['extraGuests'] . " Extra guest charges (" . $this->cart['nights'] . " nights)",
                    "price" => $this->convertedAmount($room['extraGuests'] * $roominfo->extra_guest_price * $this->cart['nights'])
                ];
            }

            $this->cart['rent'] += round($room['price']);
        }

        if($this->data['lunch']){
            $this->cart['items'][] = [
                "label" => "Lunch",
                "desc" => "Lunch PKR " . $this->convertedAmount($this->service->lunch_price) . " x " . $this->cart['persons'] . " persons x " . $this->cart['nights'] . " nights",
                "price" => $this->service->lunch_included == 'Yes' ? "FREE" : $this->convertedAmount($this->service->lunch_price * $this->cart['persons'] * $this->cart['nights'])
            ];
        }
        ...
        if($this->data['breakfast']){
            $this->cart['items'][] = [
                "label" => "Breakfast",
                "desc" => "Break Fast PKR " . $this->service->breakfast_price . " x " . $this->cart['persons'] . " persons x " . $this->cart['nights'] . " nights",
                "price" => $this->service->breakfast_included == 'Yes' ? "FREE" : $this->convertedAmount($this->service->breakfast_price * $this->cart['persons'] * $this->cart['nights'])
            ];
        }
        ...
        if($this->data['dinner'] && $this->service->dinner_included == 'No'){
            $this->cart['items'][] = [
                "label" => "Dinner",
                "desc" =>  "Dinner PKR " . $this->service->dinner_price . " x " . $this->cart['persons'] . " persons x " . $this->cart['nights'] . " nights",
                "price" => $this->service->dinner_included == 'Yes' ? "FREE" :  $this->convertedAmount($this->service->dinner_price * $this->cart['persons'] * $this->cart['nights'])
            ];
        }
        ...
        $this->cart['grandtotal'] = round($this->cart['subtotal'] - $this->cart['discounttotal']);
        ...
        // $tax = round(round($this->cart['subtotal'] - $this->cart['discounttotal']) /100 * $taxPercentage);
        $tax = 0;
        ...
        $this->cart['grandtotal'] = round($this->cart['grandtotal'] + $tax);

        $this->subtotal = round($this->cart['subtotal']);
        $this->discounttotal = round($this->cart['discounttotal']);
    ...
    {
        return true;
        // $bookingCount = Reservation::where('bookable', $this->model)
    ...
    public function sendNotification()
    {
        try {
            ...
            $notification->message=Room::TITLE;
            $notification->body=Room::MESSAGE;
            $notification->type=Room::TYPE;
            $notification->actions= Room::ACTION;
            $notification->seen=0;
