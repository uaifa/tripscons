...
class RestaurantController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if(!empty($request)){   //with('images')->
           $accommodations = Restaurant::with('images')->where(function($query) use ($proRating,$noOfPeople, $min,$max,$proType,$proSubType,$propertyFacility,$propertyThirdType,$country,$city) {
             ...
             $accommodations = Restaurant::orderBy('id', 'DESC')->paginate(\Config::get('global.pagination_records'));
        }
    ...
    public function add(Request $request)
    {
        ...
        if($accommodation->save()){
            ...
            $message    = "Restaurant service created successfully. To view detail";
            $action     = "user/setting";
            if(isset($accommodation) && !empty($accommodation)){
                PushNotification::createNotification(auth()->user(),1,$title,$message,User::TYPE_RESTAURANT,$action,$accommodation->id);
            }
    ...
    public function detail($accommodationId){
        ...
        $accomudations =  Restaurant::with('singleImage')->take(4)->get();
        $accommodation =  Restaurant::where('id',$accommodationId)->first();
